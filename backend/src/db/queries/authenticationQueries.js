...
function getUserInfo(email, callback) {
    executeQuery('SELECT * FROM authentication WHERE email = ?', email, (err, res) => {
        ...
        if (userType == 'Candidate') {
            executeQuery('SELECT * FROM candidate WHERE candidate_id = ?', userId, (err, res) => {
                if (err) {
        ...
        // If the user type is "Employer"
            ...
            executeQuery('SELECT * FROM employer WHERE employer_id = ?', userId, (err, res) => {
                if (err) {
    ...
    executeQuery('INSERT INTO authentication (email, user_type) VALUES (?, ?)', [email, userType], (err, res) => {
        ...
        if (userType == 'Candidate') {
            createCandidate(info, (err, res) => {
                ...
                executeQuery('UPDATE authentication SET candidate_id = ? WHERE email = ?', [candidate_id, email], (err, res) => {
                    if (err) {
            ...
            createEmployer(info, (err, res) => {
                ...
                executeQuery('UPDATE authentication SET employer_id = ? WHERE email = ?', [employer_id, email], (err, res) => {
                    if (err) {
